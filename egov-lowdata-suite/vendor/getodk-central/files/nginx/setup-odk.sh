...
if [ "$SSL_TYPE" = "letsencrypt" ]; then
  ...
  if [ "$SSL_TYPE" = "upstream" ]; then
    ...
    # remove the default_server block that returns 421 and make the main server accept all hostnames
    perl -i -0777 -pe 's/server \{\s+listen 80;\s+return 421;\s+\}\s+//s' /etc/nginx/conf.d/odk.conf
    perl -i -pe 's/(server_name) [^;]+;/$1 _;/' /etc/nginx/conf.d/odk.conf
